...
Последний завершённый полный прогон: **3812 тестов прошли, 0 отказов, 0 пропусков**,
...
Это состояние **до нового общего обзора объекта**. Для обзора отдельно прошли
**319 затронутых тестов**, без ошибок и пропусков, на коммите
[`484dfea`](https://github.com/blissofhaven/test123/commit/484dfea28b9740d8b4d89e44c5ff524c753e1e19).
[Новый отчёт](docs/work-verification/network-overview-2026-09-09.md) содержит проверку
навигации, запуска и читаемости мини-схем. Это отдельный целевой прогон, не новый полный набор.
...
Готов первый [общий обзор объекта](docs/network-overview.md)
на компактной учебной сети: структура слева, карточки объектов в центре,
мини-схема справа и итоги снизу. Увеличенные Т1/Т2 показывают мощность и напряжения;
доступны выбор РУ и номера фидеров с назначением при наведении. После просмотра компактного варианта следующий
согласованный шаг — группировка большого нефтепромысла и ручное назначение
принадлежности объектов; эти расширения ещё не реализованы.

![Общий обзор и читаемые Т1/Т2 в мини-схеме подстанции](docs/images/network-overview.png)
...
- Открывать общий обзор после выбора проекта и переходить к подробному листу
  двойным щелчком. Поиск и фильтр помогают найти аппарат; мини-схема служит
  только для просмотра. Перемещение обзорных карточек сохраняется с проектом
  и отменяется; подробные схемы при этом не перестраиваются. Компактный вариант
  проверен отдельно; группировка большого нефтепромысла остаётся следующим шагом.
...
После загрузки появляется «Обзор объекта». Для редактирования или КЗ откройте
нужный подробный лист; кнопка «Обзор объекта» возвращает к карточкам.
