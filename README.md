...
## Где проект сейчас — 08.09.2026

**Этапы 1–4 плана «снизу вверх» завершены в согласованном объёме.**
Последний завершённый этап — **4: топология, режимы и единый расчётный снимок**.
Следующий — **5: модели элементов и схемы последовательностей**; он ещё не начат.
Переход к нему — по отдельной команде заказчика.

Последняя полная проверка кода: **3530 тестов прошли, 0 ошибок, 0 пропусков**,
...
Обновление этой страницы меняет документацию; полный прогон относится к указанному коду.
...
Существующие МТЗ/ТО/ОЗЗ остаются переходным расчётным слоем. Перенос защит
на новые фазные токи каждого аппарата запланирован отдельно.

## Что планируется

| Этап | Следующий результат | Состояние |
|---|---|---|
| 5 | Проверенные модели источников, линий и двухобмоточных трансформаторов; схемы последовательностей | Следующий, не начат |
| 6 | Инженерная проверка всех четырёх КЗ на независимых контрольных случаях | Планируется |
| 7 | Напряжения узлов, фазные токи обоих концов ветвей и вклады источников | Планируется |
| 8 | Измерения ТТ/ТН, внутренние точки КЗ и полный набор обязательных сценариев | Планируется |
| 9 | МТЗ и ТО на новых измеряемых токах, с объяснением определяющего случая | Планируется |
