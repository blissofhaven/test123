...
Это состояние **до нового общего обзора объекта**. Его затронутые проверки
и визуальная приёмка ведутся отдельно в [новом отчёте](docs/work-verification/network-overview-2026-09-09.md);
3812 passed не выдаются за проверку последующих изменений.
...
Сейчас проверяется первый [общий обзор объекта](docs/network-overview.md)
на компактной учебной сети: структура слева, карточки объектов в центре,
мини-схема справа и итоги снизу. После просмотра компактного варианта следующий
...
- Открывать общий обзор после выбора проекта и переходить к подробному листу
  ...
  и отменяется; подробные схемы при этом не перестраиваются. Новый блок проходит
  отдельную приёмку на компактном примере.
