...
def _run_owned(net: Network, meth: Methodology, *, cancelled=None, progress=None,
    ...
    warnings: list[str] = [
        "Расчёт имеет статус ПРЕДВАРИТЕЛЬНЫЙ. Контрольные примеры подтверждают "
        "проверенные модели и условия; перед применением уставок нужна "
        "независимая проверка для конкретного объекта.",
        "В действующих расчётах защит Iк(2) — оценка √3/2·Iк(3) при Z2 = Z1. "
        "Вкладка «КЗ» рассчитывает отдельную сеть обратной последовательности "
        "по заданным Z2; её фазные токи пока не заменяют токи в расчётах защит.",
    ]
