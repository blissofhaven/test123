...
from ..model import GRID, Branch, GeneratorBranch, LineBranch, TransformerBranch
...
def calc_to(ctx: Context, br: Branch) -> ProtectionResult:
    ...
    if isinstance(br, GeneratorBranch):
        ...
        return res
    modes = [md for md in ctx.modes_where_active(br) if md.system == "max"]
    if not modes:
        modes = ctx.modes_where_active(br)
        if modes:
            res.messages.append("Нет ни одного режима с максимальной мощностью системы — "
                                "отстройка выполнена по имеющимся режимам, уставка может быть занижена.")
    if not modes:
    ...
    conditions: list[tuple[float, str, Step]] = []   # (ток отстройки, описание, шаг)

    # ── условие 1: отстройка от КЗ за пределами зоны мгновенного действия ──
    reach = br.prot.to_reach
    candidates = _transformers_at(ctx, br, modes[0])
    tr_downstream = candidates[0] if candidates else None
    if reach == "behind_transformer" and len(candidates) > 1:
        res.messages.append(
            "К концу присоединения подключено несколько трансформаторов ("
            + ", ".join(f"«{t.name}»" for t in candidates)
            + f"); отстройка выполнена по «{tr_downstream.name}». Задайте зону ТО явно.")
    if (reach == "behind_transformer" and tr_downstream is None
            and not isinstance(br, TransformerBranch)):
        # Для самого трансформатора «за трансформатором» — это его собственный
        # дальний конец, который и так является расчётной точкой; сообщение
        # выдаётся только когда трансформатор действительно не найден.
        res.messages.append(
            "Для этого присоединения выбрана отстройка ТО от КЗ за трансформатором, "
            "но в конце его зоны в данном режиме нет включённого трансформатора. "
            "Отстройка выполнена по КЗ в конце самого присоединения — проверьте "
            "схему и настройку зоны ТО.")
    if reach == "behind_transformer" and tr_downstream is not None:
        _, load_end_tr, _ = net.orient(tr_downstream, modes[0])
        target_node, target_name, kind_ru = (
            load_end_tr, net.node(load_end_tr).name,
            f"за трансформатором «{tr_downstream.name}»")
    else:
        _, load_end, _ = net.orient(br, modes[0])
        target_node, target_name, kind_ru = (
            load_end, net.node(load_end).name, f"на «{net.node(load_end).name}»")
    ...
    for mode in iter_in_declaration_order(net, modes):
        try:
            ...
            res.messages.append(f"Точка «{target_name}», режим «{mode.name}»: {e}")
            continue
        rows.append((i, mode, sc, d))
    best, tied = pick_extreme(rows, key=lambda r: r[0], largest=True)
    if best is None:
        res.messages.append("Не удалось определить ток КЗ для отстройки.")
        return res
    i_ext, mode_ext, sc_ext, d_ext = best
    tie = tie_note(tied, mode_ext.name)
    k_ots = m.k("to.k_ots")
    i_c1 = k_ots * i_ext
    st1 = Step(
    ...
    conditions.append((i_c1, "отстройка от внешнего КЗ", st1))

    # ── условие 2: бросок тока намагничивания трансформатора в зоне ──
    if tr_downstream is not None and reach == "behind_transformer":
        k_inr = m.k("to.k_ots_inrush")
        ratio = tr_downstream.i_inrush_ratio
        # Номинальный ток обмотки — паспортная величина трансформатора, а не
        # величина расчётного базиса. Среднее напряжение ступени ВЫШЕ
        # номинального (10,5 против 10), поэтому расчёт по нему занижает Iном
        # и всю уставку примерно на 5 % — то есть в сторону худшей отстройки
        # от броска. Способ выбран не здесь, а в профиле методики
        # (`to.i_nom_basis`), чтобы старые расчёты воспроизводились.
        basis = m.text("to.i_nom_basis")
        u_i_nom, basis_ru = _inrush_voltage(m, tr_downstream, u_pr, basis)
        i_nom = nominal_current(tr_downstream.s_nom, u_i_nom)
        if ratio:
            i_c2 = k_inr * ratio * i_nom
            subst = f"Iсз ≥ {fmt(k_inr)} · {fmt(ratio)} · {fmt(i_nom)} = {fmt(i_c2)} А"
            note = None
        else:
            i_c2 = k_inr * i_nom
            subst = f"Iсз ≥ {fmt(k_inr)} · {fmt(i_nom)} = {fmt(i_c2)} А"
            note = ("Кратность броска тока намагничивания для трансформатора не задана "
                    "(поле i_inrush_ratio). Условие посчитано без неё и почти наверняка "
                    "занижено — возьмите кратность из паспорта трансформатора.")
        st2 = Step(
            ...
            given={"Sном трансформатора": f"{fmt(tr_downstream.s_nom)} кВ·А",
                   "Напряжение для Iном": basis_ru,
                   "Iном (на стороне защиты)": f"{fmt(i_nom)} А",
                   "kотс.бр": fmt(k_inr),
                   "кратность броска": fmt(ratio) if ratio else "не задана"},
            formula="Iсз ≥ kотс.бр · Kбр · Iном",
            substitution=subst, result=f"Iсз ≥ {fmt(i_c2)} А",
            source=m.cite("to.k_ots_inrush"), note=note,
        )
        conditions.append((i_c2, "отстройка от броска намагничивания", st2))

    # ── выбор наибольшего условия ──
    i_calc, governing, _ = max(conditions, key=lambda c: c[0])
    for _, _, s in conditions:
        res.steps.append(s)
    if len(conditions) > 1:
    ...
    pick = Step(
        ...
        given={f"условие {i+1}: {d}": f"{fmt(v)} А" for i, (v, d, _) in enumerate(conditions)},
    ...
    res.t = br.prot.t_to if br.prot.t_to is not None else m.k("to.t")
    res.governing_mode = mode_ext.name
    ...
    for mode in iter_in_declaration_order(net, ctx.modes_where_active(br)):
        try:
    ...
    if best is None:
        ...
        if share_min <= 1e-9:
            ...
            kch = i_min / i_prim
            sens_tie = tie_note(sens_tied, mode_min.name)
            given = {
...
def _transformers_at(ctx: Context, br: Branch, mode) -> list[TransformerBranch]:
    ...
    found = [
        ...
        and b.id != br.id
        and load_end in (b.node_from, b.node_to)
    ]
    found.sort(key=lambda b: b.id)
    return found


def _reach_km(ctx: Context, br: LineBranch, i_set: float) -> float | None:
    ...
    быть корректной — такой режим пропускается, а не считается приближённо.
    """
    m, net = ctx.meth, ctx.net
    prot_node = net.protection_node_id(br)
    u_avg = stage_voltage(net.node(prot_node), m)
    worst = None
    for mode in ctx.modes_where_active(br):
        try:
            ...
            continue
        regime = mode.system
        z_line, _ = line_impedance(br, m, regime)
